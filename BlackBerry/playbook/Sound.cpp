...
bool Sound::load(const char* fileName, ALvoid* mem, ALsizei length) {
    ...
    if (mem == NULL && length == 0)
    	rawData = static_cast<unsigned char*>(alutLoadMemoryFromFile(fileName, &format, &size, &frequency));
    else
    	rawData = static_cast<unsigned char*>(alutLoadMemoryFromFileImage(mem, length, &format, &size, &frequency));
